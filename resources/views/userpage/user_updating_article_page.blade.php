...
@section('css')
<link rel="stylesheet" href="{{ URL::asset('css/userpage/user_creating_article.css') }}">
...
<div class="article-header">
  <div class="article-cover-container">
    <div class="article-cover">
      <img id="article-cover-img" src="{{ $article->cover_url }}" alt="Ảnh đại diện cho bài viết">
    </div>
...
<form action="{{ route('update_article') }}" method="post">
  ...
  <input type="text" name="cover" id="cover-input" style="display: none" value="{{ $article->cover_url }}">
  <div class="form-group">
    <div class="row">
      <div class="col-12 d-flex justify-content-center align-items-center flex-column">
        <div class="article-title mt-5">
          <input type="text" name="title" placeholder="New Document Name" class="form-text" value="{{ $article->title }}" required>
        </div>
        @if($errors->has('title'))
        <p class="error-form-control-message">
          {{ $errors->first('title') }}
...
<script type="text/javascript" src="{{ URL::asset('js/userpage/user_creating_article.js') }}"></script>
@endsection
